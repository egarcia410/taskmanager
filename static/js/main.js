...
// Edit line below with your personal api key generated from mLab
var api_key = 'u1eaJG2HTmfGAQ26AfcAShz7ZV-Lj5ml';

function getCategoryOptions(){
    $.get( "https://api.mongolab.com/api/1/databases/taskmanager/collections/categories?apiKey="+api_key, function(data){
        var output;
...
function getTasks(){
    $.get( "https://api.mongolab.com/api/1/databases/taskmanager/collections/tasks?apiKey="+api_key, function(data){
        var output = '<ul class="list-group">';
        $.each(data, function(key, data){
            output += '<li class="task list-group-item">';
            output += data.task_name+' <p><span class="due_on">[Due on '+data.due_date+']</span></p>';
                if(data.is_urgent == "yes"){
                    output += '<p><span class="label label-danger">Urgent</span></p>';
                }
            output += '<div><a class="btn btn-primary btn-edit-task" data-task-name="'+data.task_name+'" data-task-id="'+data._id.$oid+'">Edit</a> <a class="btn btn-danger btn-delete-task" data-task-id="'+data._id.$oid+'">Delete</a></div>';
            output += '</li>';
        })
        output += '</ul>';
        $("#tasks").html(output);
...
function addTask(){
    ...
    $.ajax({ url: "https://api.mongolab.com/api/1/databases/taskmanager/collections/tasks?apiKey="+api_key,
        data: JSON.stringify({
            "task_name" : task_name,
            "category": category,
            "due_date": due_date,
...
function editTask(){
    ...
    var due_date = $('#due_date').val();
    var is_urgent = $('#is_urgent').val();
    $.ajax({url: 'https://api.mongolab.com/api/1/databases/taskmanager/collections/tasks/'+task_id+'?apiKey='+api_key,
        data: JSON.stringify({
            "task_name" : task_name,
            "category": category,
            "due_date": due_date,
...
function getTask(){
    var task_id = sessionStorage.getItem('currentTaskId');
    $.get( 'https://api.mongolab.com/api/1/databases/taskmanager/collections/tasks/'+task_id+'?apiKey='+api_key, function(data){
